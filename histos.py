...
P.subplot()
P.title('KNN')
P.hist([controls[:,3], patients[:,3]],bins=bins,align='left')

P.figure(3)
P.title('SVM')
P.hist([controls[:,5], patients[:,5]],bins=bins,align='left')

P.figure(5)
P.title('AVG_all')
P.hist([controls[:,7], patients[:,7]],bins=bins,align='left')
"""
fig = 6
for g in range(1,11):
  ...
  if gesture_patients.size > 0:
    ax1.set_title('KNN')
    ax1.hist([gesture_controls[:,3], gesture_patients[:,3]],bins=bins,align='left')

    ax2.set_title('SVM')
    ax2.hist([gesture_controls[:,5], gesture_patients[:,5]],bins=bins,align='left')

    ax3.set_title('AVG')
    ax3.hist([gesture_controls[:,7], gesture_patients[:,7]],bins=bins,align='left')
"""
P.show()
